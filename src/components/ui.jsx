...
const BUTTON_VARIANTS = {
  ...
  navy: 'bg-navy-900 text-white hover:bg-navy-800 active:bg-navy-950 border border-transparent shadow-sm',
  secondary:
...
const CONTROL_BASE =
  'w-full rounded-md border bg-white px-3.5 text-sm text-navy-900 placeholder:text-slate-400 transition-colors ' +
  'focus:outline-none focus:ring-2 focus:ring-accent-500/25 focus:border-accent-500 disabled:bg-slate-50 disabled:text-slate-500'
...
export const Input = forwardRef(function Input({ className, invalid, ...props }, ref) {
  return (
    <input
      ...
      className={cx(CONTROL_BASE, 'h-11', invalid ? 'border-red-400' : 'border-slate-300', className)}
      {...props}
    />
  )
})
...
export const Textarea = forwardRef(function Textarea({ className, invalid, ...props }, ref) {
  return (
    <textarea
      ref={ref}
      aria-invalid={invalid || undefined}
      className={cx(CONTROL_BASE, 'py-2.5 min-h-24 resize-y', invalid ? 'border-red-400' : 'border-slate-300', className)}
      {...props}
    />
  )
})
...
export function Panel({ className, children, ...props }) {
  return (
    <div
      className={cx('rounded-md border border-slate-200 bg-white shadow-[var(--shadow-card)]', className)}
      {...props}
    >
      {children}
    </div>
...
export function SectionHeading({ eyebrow, title, description, align = 'center', className }) {
  return (
    <div className={cx('max-w-2xl', align === 'center' && 'mx-auto text-center', className)}>
      {eyebrow && (
        <p className="text-xs font-bold uppercase tracking-[0.14em] text-accent-600 mb-3">{eyebrow}</p>
      )}
      <h2 className="text-3xl sm:text-4xl font-bold tracking-tight text-navy-900 text-balance">{title}</h2>
      {description && <p className="mt-4 text-base sm:text-lg leading-relaxed text-slate-600">{description}</p>}
    </div>
  )
}
  ...
  const mark = (
    <span className="inline-flex items-center gap-2.5">
      ...
      <span className={cx('font-bold tracking-tight', size === 'sm' ? 'text-base' : 'text-lg')}>
        ...
        <span className={invert ? 'text-white' : 'text-navy-900'}>Card</span>
        <span
...
export function Switch({ checked, onChange, label, description, locked = false, onLocked }) {
  ...
  return (
    <div className="flex items-start justify-between gap-6">
      <div>
        <label htmlFor={id} className="inline-flex items-center gap-2 text-sm font-semibold text-navy-900">
          {label}
          {locked && (
            <Badge tone="slate">
      ...
      >
        <span
          className={cx(
            'absolute top-0.5 h-4 w-4 rounded-xs bg-white shadow transition-all',
